...
namespace KCKantanInstallation
{
	...
	{
	public:
		static FKantanPluginInstallationImpl& Initialize(const FString& InPluginName, bool bHasUserFocusedContent);
	...
	private:
		...
		void InternalInitialize(bool bHasUserFocusedContent);
		...
		bool SetupKantanPluginInfoAsset() const;
		bool SetupKantanPluginDocsAsset() const;
		void RegisterPopup(bool bHasUserFocusedContent) const;

		static TSharedRef< SWidget > CreateMessageWidget(const FText& Message, const FName& TextStyleName = NAME_None, const FMargin& Margin = FMargin(0), const float LineHeight = 1.0f);
		...
		virtual bool DisplayEditorPopup(const FText& WindowTitle, TSharedRef< SWidget > Content) const override;
		virtual bool DisplayDefaultKantanEditorPopup(EKantanInfoShowReason Reason, bool bHasUserFocusedContent) const override;
		virtual void ShowPluginDocumentation() const override;
		virtual bool ConditionallyExecuteWithConfigCheck(TFunction< bool() > Func, const FString& CheckIdentifier, FConfigCheckOp& CheckOperator) const override;
		/**/
	...
	FKantanPluginInstallationImpl& FKantanPluginInstallationImpl::Initialize(const FString& InPluginName, bool bHasUserFocusedContent)
	{
		if (Instance.IsValid() == false)
		{
			Instance = MakeUnique< FKantanPluginInstallationImpl >(InPluginName);
			Instance->InternalInitialize(bHasUserFocusedContent);
		}
	...
	void FKantanPluginInstallationImpl::InternalInitialize(const bool bHasUserFocusedContent)
	{
		FKantanInstallationStyleSet::Initialize(PluginName);

		InitializeKantanInfoAssetType();
		SetupKantanPluginInfoAsset();
		SetupKantanPluginDocsAsset();

		RegisterPopup(bHasUserFocusedContent);
	}
	...
	void FKantanPluginInstallationImpl::InitializeKantanInfoAssetType()
	{
		...
		KantanInfoAssetActions = MakeShareable(new FAssetTypeActions_KantanInfo());
	...
	UKCKantanInfoAssetBase* FKantanPluginInstallationImpl::CreateInfoAsset(TSubclassOf< UKCKantanInfoAssetBase > Class, const FString& AssetName) const
	{
		...
		// at the same path.
		const auto AssetPackage = CreatePackage(nullptr, *AssetPackageName);
	...
	bool FKantanPluginInstallationImpl::SetupKantanPluginInfoAsset() const
	{
		...
		{
			Info->DefaultAction.BindLambda([]
			{
				if (FKantanPluginInstallationImpl::IsAvailable())
				{
					FKantanPluginInstallationImpl::Get().DisplayDefaultKantanEditorPopup(EKantanInfoShowReason::DirectRequest, false);
				}
			});
			return true;
		}
	...
	void FKantanPluginInstallationImpl::RegisterPopup(bool bHasUserFocusedContent) const
	{
		...
		if (Plugin.IsValid())
		{
			...
			TSharedRef< EKantanInfoShowReason > ShowReason = MakeShareable(new EKantanInfoShowReason(EKantanInfoShowReason::DirectRequest));
			...
			const auto ShowPopup = [ShowReason, bHasUserFocusedContent]() -> bool
			{
				if (FKantanPluginInstallationImpl::IsAvailable())
				{
					return FKantanPluginInstallationImpl::Get().DisplayDefaultKantanEditorPopup(*ShowReason, bHasUserFocusedContent);
				}
				return false;
			};
	...
	TSharedRef< SWidget > FKantanPluginInstallationImpl::CreateKantanLinksWidget(const FText& ProductName, const FString& DocsUrl)
	{
		...
		const FVector2D IconSize(32, 32);
	...
	TSharedRef< SWidget > FKantanPluginInstallationImpl::CreatePopupOnVersionUpdateToggle()
	{
		...
		TSharedRef< ECheckBoxState > CheckState = MakeShareable(new ECheckBoxState(bShouldPopup ? ECheckBoxState::Checked : ECheckBoxState::Unchecked));
	...
	bool FKantanPluginInstallationImpl::DisplayDefaultKantanEditorPopup(const EKantanInfoShowReason Reason, const bool bHasUserFocusedContent) const
	{
		const auto Plugin = GetPlugin();
		if (Plugin.IsValid())
		{
			...
			const auto PluginIconBrush = (const FSlateBrush*)nullptr;	// @TODO: reenable when available FKantanInstallationStyleSet::Get().GetBrush(TEXT("Icon.PluginLogo"));
			...
			TopMessage = TopMessage.IsValid() ? SNew(SBox).Padding(FMargin(0, 15, 0, 0))[TopMessage.ToSharedRef()] : SNullWidget::NullWidget;
				...
				[
					...
					[
						...
						[
							...
							+ SVerticalBox::Slot().FillHeight(1).VAlign(EVerticalAlignment::VAlign_Center)
							[
	...
	IKantanPluginInstallation& InitializeKantanPluginInstallation(const FString& PluginName, const bool bHasUserFocusedContent)
	{
		return FKantanPluginInstallationImpl::Initialize(PluginName, bHasUserFocusedContent);
	}
