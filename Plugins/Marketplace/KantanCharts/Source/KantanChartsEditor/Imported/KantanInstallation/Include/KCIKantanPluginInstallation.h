...
namespace KCKantanInstallation
{
	...
	public:
		...
		virtual bool DisplayDefaultKantanEditorPopup(EKantanInfoShowReason Reason, bool bHasUserFocusedContent) const = 0;
		...
		struct FConfigCheckOp
		{
			...
			inline bool PerformCheck(FConfigFile& Config, const FString& Section, const FString& Key) const
			{
				return !_PerformCheck || _PerformCheck(Config, Section, Key);
			}

			inline void PostOperation(FConfigFile& Config, const FString& Section, const FString& Key) const
			{
				if (_PostOperation)
				{
			...
			static inline FConfigCheckOp AlwaysPass()
			{
				return FConfigCheckOp();
			}

			static inline FConfigCheckOp BoolCheck(const bool bPassIfMissing)
			{
	...
	IKantanPluginInstallation& InitializeKantanPluginInstallation(const FString& PluginName, bool bHasUserFocusedContent);
